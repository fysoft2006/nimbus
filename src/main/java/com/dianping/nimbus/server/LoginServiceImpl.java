...
import com.dianping.nimbus.client.service.LoginService;
...
import com.dianping.nimbus.server.store.service.UserLoginService;
...
public class LoginServiceImpl extends RemoteServiceServlet implements
	...
	private static Cache<String, String> usernameToPasswd = CacheBuilder
			.newBuilder().concurrencyLevel(4).maximumSize(5000)
			.expireAfterWrite(24, TimeUnit.HOURS)
			.removalListener(new RemovalListener<String, String>() {
				@Override
				public void onRemoval(RemovalNotification<String, String> rn) {
					logger.info(("username:" + rn.getKey() + " password:"
							+ rn.getValue() + " was removed from usernameToPasswd"));
				}
			}).build();
	...
	public static Cache<String, String> getUsernameToPasswd() {
		return usernameToPasswd;
	}
	...
	public LoginTokenBo authenticate(String username, String password) {
		...
		if (ugi == null) {
			...
			if (conn != null) {
				...
				usernameToPasswd.put(username, password);
