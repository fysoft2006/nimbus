...
import com.dianping.nimbus.client.service.HiveQueryService;
import com.dianping.nimbus.server.mail.MailService;
import com.dianping.nimbus.server.queryengine.IQueryEngine;
import com.dianping.nimbus.server.queryengine.cmdline.HiveCmdLineQueryEngine;
import com.dianping.nimbus.server.queryengine.jdbc.DataFileStore;
import com.dianping.nimbus.server.queryengine.jdbc.HiveJdbcClient;
import com.dianping.nimbus.server.queryengine.jdbc.JdbcQueryEngine;
import com.dianping.nimbus.server.queryengine.rest.RestQueryEngine;
import com.dianping.nimbus.server.store.domain.QueryFavorite;
import com.dianping.nimbus.server.store.domain.QueryHistory;
import com.dianping.nimbus.server.store.service.QueryFavoriteService;
import com.dianping.nimbus.server.store.service.QueryHistoryService;
...
public class HiveQueryServiceImpl extends RemoteServiceServlet implements
	...
	public HiveQueryOutputBo getQueryResult(HiveQueryInputBo input) {
		...
		if (queryEngine instanceof JdbcQueryEngine) {
			...
			output = queryEngine.getQueryResult(input);
		} else if (queryEngine instanceof RestQueryEngine) {
			output = queryEngine.getQueryResult(input);
		} else {
	...
	public Boolean submitQueryError(QueryErrorBo error) {
		...
		LOG.info("sending hive bug mail from user " + error.getUsername());
